 ...
 * {@code /chaosaholic-notify status | sound|message|effects [on|off]}. A setting without a value is flipped.
 * The root is not {@code /chaosaholic ...} so it cannot clash with the server's op command.
 */
public final class NotifyCommand {
	public static final String ROOT = "chaosaholic-notify";
	...
	public static void register() {
		ClientCommandRegistrationCallback.EVENT.register((dispatcher, buildContext) -> dispatcher.register(
				ClientCommands.literal(ROOT)
						...
						.then(setting("effects", NotifySettings::effects, NotifySettings::withEffects))));
	}
	...
	private static void feedback(CommandContext<FabricClientCommandSource> c, String name, boolean value) {
		c.getSource().sendFeedback(Component.translatable("chaosaholic.command.notify." + name, CommonComponents.optionStatus(value)));
	}
}
