...
/**
 * moon_jump: Jump Boost III and no fall damage (ender pearls still hurt) for the affected player only; both end with
 * the event, with a Slow Falling tail for a player still in the air. Mock players are invulnerable, so damage is
 * checked through the Fabric ALLOW_DAMAGE event (what LivingEntity#hurt asks).
 */
public class MoonJumpGameTests {
	...
	public void expiresAndCleansUp(GameTestHelper h) {
		...
		h.startSequence()
				.thenExecute(() -> {
					p.setOnGround(false);
					p.fallDistance = 20.0; // jumped off a cliff under the event
				})
	...
	@GameTest
	public void deathGetsNoTail(GameTestHelper h) {
		defaults(h);
		ServerPlayer p = survivalPlayer(h);
		ActiveEvent ev = start(h, "moon_jump", p);
		p.setOnGround(false);
		p.fallDistance = 20.0;
		manager(h).removePlayer(ev, p, RemoveReason.DEATH);
		h.assertFalse(p.hasEffect(MobEffects.SLOW_FALLING), "no tail on death");
		cleanup(h, p);
		h.succeed();
	}
}
