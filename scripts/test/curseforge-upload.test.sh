...
# dummy build outputs
printf 'jar' > "$tmp/chaosaholic-1.2.0-beta.1.jar"
printf 'src' > "$tmp/chaosaholic-1.2.0-beta.1-sources.jar"
...
run_upload \
  ...
  bash "$script" "$tmp/chaosaholic-1.2.0-beta.1.jar" "$tmp/chaosaholic-1.2.0-beta.1-sources.jar"
if [[ $rc -ne 0 ]]; then
...
# (b) relations (the same value as release.yml passes)
if jq -e '.relations.projects == [{"slug":"fabric-api","type":"requiredDependency"},{"slug":"modmenu","type":"optionalDependency"}]' <<<"$primary" >/dev/null; then
  pass "(b) relations contain fabric-api requiredDependency and modmenu optionalDependency"
else
...
if jq -e '.releaseType == "beta" and .changelog == "## Changes" and .changelogType == "markdown"
          and .displayName == "chaosaholic-1.2.0-beta.1.jar"' <<<"$primary" >/dev/null; then
...
if jq -e 'has("parentFileID") and (has("gameVersions") | not)
          and .displayName == "chaosaholic-1.2.0-beta.1-sources.jar"' <<<"$child" >/dev/null; then
...
run_upload CF_GAME_VERSIONS='26.9,Fabric' bash "$script" "$tmp/chaosaholic-1.2.0-beta.1.jar"
...
# --- guards ---------------------------------------------------------------------------------
run_upload CF_DRY_RUN=false CF_GAME_VERSIONS='26.2' bash "$script" "$tmp/chaosaholic-1.2.0-beta.1.jar"
...
run_upload CF_GAME_VERSIONS='26.2' CF_RELATIONS='fabric-api:needed' bash "$script" "$tmp/chaosaholic-1.2.0-beta.1.jar"
...
run_upload CF_TYPE_PREFIXES='' CF_TYPES_JSON=/nonexistent CF_VERSIONS_JSON="$here/fixtures/versions-bedrock.json" \
  CF_GAME_VERSIONS='26.50' bash "$script" "$tmp/chaosaholic-1.2.0-beta.1.jar"
...
run_upload CF_TYPES_JSON="$tmp/empty-types.json" CF_GAME_VERSIONS='26.2' bash "$script" "$tmp/chaosaholic-1.2.0-beta.1.jar"
...
run_upload CF_GAME_VERSIONS=' , ' bash "$script" "$tmp/chaosaholic-1.2.0-beta.1.jar"
...
run_upload CF_RELEASE_TYPE=rc CF_GAME_VERSIONS='26.2' bash "$script" "$tmp/chaosaholic-1.2.0-beta.1.jar"
...
run_upload PATH="$tmp/nojq" CF_GAME_VERSIONS='26.2' bash "$script" "$tmp/chaosaholic-1.2.0-beta.1.jar"
...
if command -v python3 >/dev/null; then
  ...
  : > "$tmp/github_output"
  run_real CF_CHANGELOG="$changelog" CF_DISPLAY_NAME='Chaosaholic 1.2.0 "beta" (Fabric)' \
    CF_GAME_VERSIONS=$'26.2\nFabric' CF_RELATIONS=' fabric-api : requiredDependency , modmenu:optionalDependency ' \
    bash "$script" "$tmp/chaosaholic-1.2.0-beta.1.jar" "$tmp/chaosaholic-1.2.0-beta.1-sources.jar" "$tmp/extra dir/my mod-1.0.0-extra.jar"
  ...
  if [[ $rc -eq 0 ]] && jq -e --arg c "$changelog" '.metadata.changelog == $c and .token == "test-token"
        ...
        and .metadata.displayName == "Chaosaholic 1.2.0 \"beta\" (Fabric)" and .metadata.gameVersions == [7499,16498]
        and .metadata.relations.projects == [{"slug":"fabric-api","type":"requiredDependency"},{"slug":"modmenu","type":"optionalDependency"}]
        and .filename == "chaosaholic-1.2.0-beta.1.jar" and .file == "jar"' "$u0" >/dev/null; then
  ...
  run_real CF_GAME_VERSIONS='26.2' bash "$script" "$tmp/chaosaholic-1.2.0-beta.1.jar"
  ...
  run_real CF_TYPE_PREFIXES='' CF_GAME_VERSIONS='26.50' bash "$script" "$tmp/chaosaholic-1.2.0-beta.1.jar"
  ...
  started=$SECONDS
  run_real CF_TOKEN=wrong CF_GAME_VERSIONS='26.2' bash "$script" "$tmp/chaosaholic-1.2.0-beta.1.jar"
  ...
  run_upload CF_DRY_RUN=false CF_TOKEN=test-token CF_API_BASE="http://127.0.0.1:1" CF_RETRY_DELAY=0 \
    CF_GAME_VERSIONS='26.2' bash "$script" "$tmp/chaosaholic-1.2.0-beta.1.jar"
...
if (
  shopt -s nullglob
  cd "$tmp"
  primary_glob=( chaosaholic-!(*-sources).jar )
  template_glob=( !(*-sources).jar )
  [[ "${primary_glob[*]}" == "chaosaholic-1.2.0-beta.1.jar" && "${template_glob[*]}" == "chaosaholic-1.2.0-beta.1.jar" ]]
); then
